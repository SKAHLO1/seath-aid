...
// Simulated verifier traffic. Every issuer and verifier in this demo is
// synthetic and every medical value is invented — this feed illustrates the
// SHAPE of the system (who asks, which circuit runs, what comes back), not real
// activity. Names are deliberately obviously-fake handles.
const AGENT_NAMES = [
  "pharmacy-7f2a", "employer-3b1c", "insurer-9d4e", "clinic-2c8f",
  "travel-5a3d", "school-1e9b", "lab-4f2c", "hospital-8d1a",
  "registrar-6b3e", "gym-0c7f",
]

// What a verifier asked to have proven. Each maps to a real circuit in
// contracts/src/verihealth.compact.
const TASKS = [
  "Vaccination complete as of today",
  "Lab value below clinical threshold",
  "Coverage active on date of service",
  "Vaccination — required dose count met",
  "Lab value above minimum threshold",
  "Policy in force, expiry not passed",
  "Vaccination status for travel entry",
  "Coverage check before admission",
  "Threshold check — no value revealed",
  "Credential not present in revocation set",
  "Nullifier unspent — replay rejected",
  "Issuer attestation signature valid",
]

// The circuit that actually ran, rather than a datacentre region.
const REGIONS = ["vaccination", "labThreshold", "coverage"]

// Proof outcomes. "revoked" is a genuine outcome here, not an error: the
// circuit checked the on-chain revocation registry and refused.
const STATUSES = [
  { label: "verified", color: "#4ade80" },
  { label: "verified", color: "#4ade80" },
  { label: "verified", color: "#4ade80" },
  { label: "proving",  color: "#facc15" },
  { label: "revoked",  color: "#f87171" },
]
...
function randomRow(key: number): AgentRow {
  return {
    ...
    elapsed: `${Math.floor(Math.random() * 9 + 1)}.${Math.floor(Math.random() * 9)}s`,
    key,
  }
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {["VERIFIER", "CLAIM PROVEN", "CIRCUIT", "RESULT"].map(h => (
